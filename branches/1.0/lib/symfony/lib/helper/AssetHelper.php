 ...
 * @version    SVN: $Id: AssetHelper.php 3280 2007-01-15 07:06:05Z fabien $
 */
...
function image_tag($source, $options = array())
{
  ...
  if (isset($options['thumb_width']) || isset($options['thumb_height'])) {
  	$options['src'] = '/php/phpThumb.php?src='.$source;
  	if (isset($options['thumb_width'])) $options['src'] .= '&w='.$options['thumb_width'];
  	if (isset($options['thumb_height'])) $options['src'] .= '&h='.$options['thumb_height'];
  	unset($options['thumb_width']);
  	unset($options['thumb_height']);
  }
