...
function object_select_tag($object, $method, $options = array(), $default_value = null)
{
  ...
  $peer_method_arg = isset($options['peer_method_arg']) ? $options['peer_method_arg'] : null;
  unset($options['peer_method_arg']);

  $select_options = _get_options_from_objects(sfContext::getInstance()->retrieveObjects($related_class, $peer_method, $peer_method_arg), $text_method);
