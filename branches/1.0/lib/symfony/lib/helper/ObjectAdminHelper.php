 ...
 * @version    SVN: $Id: ObjectAdminHelper.php 3240 2007-01-12 08:56:48Z chtito $
 */

function object_admin_input_file_tag($object, $method, $options = array())
{
	$not_picture = false;
	if (@$options['not_image']) $not_picture = true;
	unset($options['not_image']);
  ...
  if ($value)
  {
    if ($include_link = _get_option($options, 'include_link'))
    {
      ...
      if ($not_picture) {
      	$html .= sprintf('<a href="%s" target="_blank">%s</a>', $image_path, $image_text)."\n";
      } else {
      	$html .= sprintf('<a href="%s" rel="lightbox">%s</a>', $image_path, $image_text)."\n";
      }
    }
