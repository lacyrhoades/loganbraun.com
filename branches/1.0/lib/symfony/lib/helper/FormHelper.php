 ...
 * @version    SVN: $Id: FormHelper.php 3294 2007-01-16 06:53:15Z fabien $
 */
...
/**
 * Select tag for states
 */
function select_state_tag($name, $selected = null, $options = array())
{
	$c = new sfCultureInfo(sfContext::getInstance()->getUser()->getCulture());
  $states = $c->getStates();

  if ($state_option = _get_option($options, 'states'))
  {
    foreach ($states as $key => $value)
    {
      if (!in_array($key, $state_option))
      {
        unset($states[$key]);
      }
    }
  ...
  $option_tags = options_for_select($states, $selected, $options);
